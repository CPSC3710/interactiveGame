...
void Cylinder::draw() {
  ...
  glTranslatef(0, 0, 1);
  glColor3f(1, 1, 0);
  gluDisk(this->m_quad, 0, 0.25, 16, 10);
  glPopMatrix();
  ...
  gluCylinder(this->m_quad, 0.25, 0.25, 1.0, 16, 10);
  glPopMatrix();
...
void Cylinder::setPersistent() {
   this->m_isDestroyable = false;
}

bool Cylinder::isDestroyable() const {
   return this->m_isDestroyable;
}
